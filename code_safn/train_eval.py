...
# Target: test set
test_set_target = DatasetGeneratorMultimodal(data_root_target, split_target, ds_name='ROD', do_rot=False, transform=test_transform)
...
for epoch in range(first_epoch, args.epochs + 1):
    ...
    with tqdm(total=len(train_loader_source), desc="Training: ") as pb:
        ...
        for batch_num, (img_rgb_source, img_depth_source, img_label_source) in enumerate(train_loader_source_rec_iter):
            ...
            with OptimizerManager(optims_list):
                ...
                # SAFN
                l2norm_loss_source = l2norm_loss(fc_source)
                l2norm_loss_target = l2norm_loss(fc_target)
    ...
    #Log SAFN feature
    writer.add_scalar("MeanFeature/feat_source", l2norm_mean(fc_source).item(), epoch) # mean feature source after epoch i
    writer.add_scalar("MeanFeature/feat_target", l2norm_mean(fc_target).item(), epoch) # mean feature target after epoch i
