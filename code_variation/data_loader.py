...
def get_relative_rotation(rgb_rot, depth_rot):
    ...
    assert rel_rot in Interval(0, 2 * math.pi)
    return rel_rot
...
class DatasetGeneratorMultimodal(Dataset):
    ...
    def __getitem__(self, index):
        ...
        rot_rgb = random.uniform(0, 2 * math.pi) if self.do_rot else None;
        rot_depth = random.uniform(0, 2 * math.pi) if self.do_rot else None;
